...
const Bartenders = Router()

Bartenders.get("/bartenders", async (req, res) => {
    const employees = await Employee.find({})
    res.render("Bartenders", { employees })
})

Bartenders.post("/bartenders", async (req, res) => {
...
Bartenders.put("/bartenders/:id", async (req, res) => {
    const { id } = req.params
...
Bartenders.delete("/bartenders/:id", async (req, res) => {
    let { id } = req.params
    await Employee.deleteOne({ _id: id })
    res.redirect("/bartenders")
})

module.exports = Bartenders